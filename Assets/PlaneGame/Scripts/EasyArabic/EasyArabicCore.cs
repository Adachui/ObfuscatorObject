...
namespace EasyAlphabetArabic
{
    public static class EasyArabicCore
    {
		...
		static numCase NumCase = numCase.ARABIC;
        ...
        public static string CorrectString(string text, int numsFormat = 0)
        {
			...
			if ((int)NumCase < 0 || (int)NumCase > 2) NumCase = numCase.ARABIC; // wrong input, fallback to latin
...
//			var lines = text.Split(new string[] { "\n" }, StringSplitOptions.RemoveEmptyEntries);
        ...
        static string CorrectText(List<string> inputStrings, bool TMPro = false)
        {
            ...
            for (int i = 0; i < inputStrings.Count; i++)
            {
                ...
                else
                {
					...
					inputStrings[i] = EasyArabicInternals.Correct(inputStrings[i].ToCharArray(), IsNum (inputStrings [i]) ? 0 : (int)NumCase); // correct the string
		...
		static bool IsNum(string txt)
		{
			foreach (var c in txt)
				if ((c >= 48 && c <= 57) || c == 44) return true;
			return false;
		}
        ...
        static string CorrectBetweenTags(string text, bool TMPro = false)
        {
            ...
            {
				betweenStr = EasyArabicInternals.Correct(betweenStr.ToCharArray(), IsNum (betweenStr) ? 0 :  (int)NumCase);
